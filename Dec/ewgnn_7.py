...
from bp_7 import atanh2, tanh1_2
from utils import compute_ber, compute_bler


class NodeInfoUpdate_GNN(Layer):
...
class GNN_Decoder(Layer):
    ...
    def build(self, input_shape):
        ...
        self.update_h_m = NodeInfoUpdate_GNN(output='M',
                                             gather_index=self._hm_update_edges)
        self.update_h_e = NodeInfoUpdate_GNN(output='E',
                                             gather_index=self._he_update_edges)
        self.readout = NodeInfoUpdate_GNN(output='LLR',
...
def test_model_gnn(model, params):
    ...
    for i in range(num_points.numpy()):
        ...
        print(f"SNR {snr_dbs[i]}, ber = {bers[i]:.5e}, bler = {blers[i]:.5e}, " \
              f"block_num = {num_batch * block_per_batch:d}," \
              # f"run_time = {runtime:.2f}s"
              )

    return bers, blers


def train_model_gnn(model, params):
    loss = tf.keras.losses.BinaryCrossentropy()

    for p in params:
        trian_snr_low, train_snr_high, train_batch_size, lr, train_iter = p
        optimizer = tf.keras.optimizers.Adam(learning_rate=lr)

        @tf.function()
        def train_step():
            ebno_db = tf.random.uniform([train_batch_size, 1], minval=trian_snr_low, maxval=train_snr_high)
            with tf.GradientTape() as tape:
                ...
                for m, l in enumerate(llr_hat):
                    i += 1
                    p1_hat = tf.sigmoid((-1) * l)
                    loss_value += loss(c, p1_hat)
                loss_value = loss_value / i
        ...
        for i in range(train_iter):
            loss = train_step()
            if i % 100 == 0:
                print(f"Step {i}, loss = {loss.numpy():.6f}")
                bers, blers = test_model_gnn(model, [6.0, 6.0, 1000, 5000, 500])
                print(f"ber: {bers}")
                print(f"bler: {blers}")